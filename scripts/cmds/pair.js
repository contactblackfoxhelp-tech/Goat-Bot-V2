const axios = require("axios");
    ...
    onStart: async function({ api, event, usersData }) {
      try {
        ...
        if (!myData || !myData.gender) {
          api.sendMessage("Could not determine your gender.", event.threadID, event.messageID);
        }
        ...
        if (myGender === "MALE") {
          ...
          api.sendMessage("Your gender is undefined. Cannot find a match.", event.threadID, event.messageID);
        }
        if (matchCandidates.length === 0) {
        api.sendMessage("No suitable match found in the group.", event.threadID, event.messageID);
        }
  ...
  out.on('finish', () => {
    api.sendMessage({
      body: `${senderName} has been paired with ${matchName}!`,
      ...
      fs.unlinkSync(outputPath);
        ...
        api.sendMessage("An error occurred while trying to find a match." + error.message, event.threadID, event.messageID);
      }
    }
  };
