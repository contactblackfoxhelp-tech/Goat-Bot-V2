  ...
  onStart: async function ({ api, event, message, args}) {
    ...
    if (args[0]?.startsWith("https://") || args[0]?.startsWith("http://")) {
      try {
        ...
        return message.reply("𝙸𝙽𝚅𝙰𝙻𝙸𝙳 𝙵𝙰𝙲𝙴𝙱𝙾𝙾𝙺 𝙻𝙸𝙽𝙺 𝙾𝚁 𝙿𝚁𝙾𝙵𝙸𝙻𝙴 𝙽𝙾𝚃 𝙵𝙾𝚄𝙽𝙳...");
      }
    ...
    const loadingSteps = [
      ...
      "🔎 𝙷𝙰𝙲𝙺𝙸𝙽𝙶█▓▒­░▒▒▒▒▒⡷⠂",
      "🔎 𝙷𝙰𝙲𝙺𝙸𝙽𝙶█▓▒­░▒▒▒▒▒▒⡷⠂",
      "🔎 𝙷𝙰𝙲𝙺 𝙲𝙾𝙼𝙿𝙻𝙴𝚃𝙴" // The 🔎 is now at the end
    ];
    ...
    setTimeout(async () => {
      await message.reply("𝙲𝚈𝙱𝙴𝚁 𝚂𝙴𝙲𝚄𝚁𝙸𝚃𝚈 𝙲𝚁𝙰𝙲𝙺𝙴𝙳 𝚂𝚄𝙲𝙲𝙴𝚂𝙵𝚄𝙻𝙻𝚃_🔓");
    }, 6000);

    setTimeout(async () => {
      await message.reply("𝚆𝙰𝙸𝚃 𝙰 𝙵𝙴𝚆 𝚂𝙴𝙲𝙾𝙽𝙳𝚂...⏱");
    }, 6500);
